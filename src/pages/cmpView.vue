    ...
    <div q-card class="my-card">
      ...
      <div class="row" v-if="codedir && db">
        <div class="col-12">
              ...
              <q-splitter v-model="splitterModel" v-if="columns.length > 0">
                ...
                <template v-slot:before>
                  <q-tabs v-model="tabsource" vertical style="width: auto">
                    ...
                    <q-tab name="SPRING-BOOT" label="SPRING-BOOT" />
                  ...
                  >
                    ...
                    <q-tab-panel name="SPRING-BOOT">
                      ...
                      <monaco-editors
                        ...
                        ext="java"
                        type="SpringBoot"
                        :vhtml="setSpringbootController(columns)"
                      ></monaco-editors>
                      ...
                      <monaco-editors
                        ...
                        ext="java"
                        type="SpringBoot"
                        :vhtml="setSpringbootSvc(columns)"
                      ></monaco-editors>
                      ...
                      <monaco-editors
                        ...
                        ext="java"
                        type="SpringBoot"
                        :vhtml="setSpringbootSvcImpl(columns)"
                      ></monaco-editors>
                      ...
                      <monaco-editors
                        :config="config"
                        title="Update-Req"
                        :sample="'Update' + sample + 'Request'"
                        ext="java"
                        type="SpringBoot-DTO"
                        :vhtml="setSpringbootUpdateReq(columns)"
                      ></monaco-editors>
                      <!-- InsertReq -->
                      <monaco-editors
                        :config="config"
                        title="Insert-Req"
                        :sample="'Insert' + sample + 'Request'"
                        ext="java"
                        type="SpringBoot-DTO"
                        :vhtml="setSpringbootInsertReq(columns)"
                      ></monaco-editors>
                      <!-- SearchReq -->
                      <monaco-editors
                        :config="config"
                        title="Search-Req"
                        :sample="'Search' + sample + 'Request'"
                        ext="java"
                        type="SpringBoot-DTO"
                        :vhtml="setSpringbootSearchReq(columns)"
                      ></monaco-editors>
                      <!-- SelectRes -->
                      <monaco-editors
                        :config="config"
                        title="Select-Res"
                        :sample="'Select' + sample + 'Response'"
                        ext="java"
                        type="SpringBoot-DTO"
                        :vhtml="setSpringbootSelectRes(columns)"
                      ></monaco-editors>
                      <!-- setSpringBootDao -->
                      <monaco-editors
                        :config="config"
                        title="SpringBoot_Dao"
                        :sample="sample + 'DAO'"
                        ext="java"
                        type="SpringBoot"
                        :vhtml="setSpringBootDao(columns)"
                      ></monaco-editors>
                      <!-- setSpringBootMapperXml -->
                      <monaco-editors
                        :config="config"
                        title="SpringBoot_Mapper_Xml"
                        :sample="sample"
                        ext="xml"
                        type="SpringBoot-XML"
                        :vhtml="setSpringBootMapperXml(columns)"
                      ></monaco-editors>
                      <!-- setSpringBootVue -->
                      <monaco-editors
                        :config="config"
                        title="cmppotal vue component"
                        :sample="sample+'View'"
                        ext="xml"
                        type="SpringBoot-VUE"
                        :vhtml="setSpringBootVue(columns)"
                      ></monaco-editors>
                    </q-tab-panel>
                  </q-tab-panels>
                </template>
          ...
          <q-tab-panel name="add">
            <div class="row">
              <div class="col-12" v-for="(item, key, index) in selectedtables" :key="key">
                <table>
                  <tr></tr>
                </table>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      config: {
        ...
        packagenm: localStorage.getItem("packagenm"),
  ...
  methods: {
    ...
    getConfig() {
      ...
      this.config.dir_front= localStorage.getItem('dir_front');*/
      this.config.db_type = this.db;
      let prj_root = this.config.packagedir.split(".").reverse()[0].trim();
      ...
      this.config.dir_xml_config =
        "/src/main/resources/" + this.config.packagenm + "/mapper/config/";
      // this.config.dir_validator =
      //   "/src/main/" + this.config.packagenm + "/com/";
      this.config.dir_validator = "/src/main/" + this.config.packagenm + "/" + prj_root;
      // this.config.dir_jsp =
       ...
       * TS
       */
      this.config.dir_springboot = `${this.config.dir_java}${this.sample}/`;
      this.config.dir_springboot_xml = `/src/main/resources/mybatis/mapper/`;
      this.config.dir_springboot_dto = `${this.config.dir_java}${this.sample}/dto/`;
      this.config.dir_springboot_vue = `/frontend/src/views/`;
      /**
       *
       */
    },
    ...
    openPlayLoad: function (items) {
       ...
       * TS
       */
      let updateTabledobj, insertTabledobj;
      /*try {
      ...
      // 기본키 및 조건 절 세팅
      if (this.prinameobj) {
        //TS :prikey arg (type1 name1,type2 name2 ,,,)
        prikeyParams = this.prinameobj
          .map(
            (e) => this.dataTypeinitTuTb(e.data_type, 2) + " " + camelCase(e.column_name)
          )
          .join(",");
        prikeyArgs = this.prinameobj.map((e) => camelCase(e.column_name)).join(",");
      ...
      //   e => "\n\t\t\t" + e.column_name
      // );
      console.log("####inserttable#####", this.insertTabledobj);
      ...
      let insertBox2 = this.insertTabledobj
        .map((e) => this.vueInsertBox2(e, SampleSmIdx, SampleSmIdxs))
        .join("\n "); // insert 폼
      ...
      /**
       * TS vaild java server
       */
      console.log(
        "@#@#@#@#@#",
        this.selectedNotNullsobj,
        this.selectedNotNullsEditobj,
        "@#@#@#@#@#"
      );
      insertTabledobj = this.insertTabledobj.map((data) => {
        const found = this.selectedNotNullsobj.find(
          (e) => e.column_name == data.column_name
        );
        return { ...data, validNotNull: found != null };
      });
      updateTabledobj = this.updateTabledobj.map((data) => {
        const found = this.selectedNotNullsEditobj.find(
          (e) => e.column_name == data.column_name
        );
        return { ...data, validNotNull: found != null };
      });
      ...
      let payload = {
         ...
         * TS
         */
        selectedSearchobj: this.selectedSearchobj,
        insertTabledobj: insertTabledobj,
        updateTabledobj: updateTabledobj,
        selectedObj: this.selectedObj,
        prikeyParams: prikeyParams,
        prikeyArgs: prikeyArgs,
      };
      //console.log(payload);
     ...
     * #TS spring-boot
     */
    setSpringbootController: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootController.getCtrl(payload, this);
    },
    setSpringbootSvc: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootSvc.getCtrl(payload, this);
    },
    setSpringbootSvcImpl: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootSvcImpl.getCtrl(payload, this);
    },
    setSpringbootUpdateReq: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootUpdateReq.getCtrl(payload, this);
    },
    setSpringbootInsertReq: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootInsertReq.getCtrl(payload, this);
    },
    setSpringbootSearchReq: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootSearchReq.getCtrl(payload, this);
    },
    setSpringbootSelectRes: function (items) {
      let payload = this.setPlayLoad(items);
      return this.springbootSelectRes.getCtrl(payload, this);
    },
    setSpringBootDao: function (items) {
      let payload = this.setPlayLoad(items);
      let r = this.springBootDao.getDao(payload, this);
      return r;
    },
    setSpringBootMapperXml: function (items) {
      let payload = this.setPlayLoad(items);
      let r = this.springBootMapperXml.getMapperXml(payload);
      return r;
    },
    setSpringBootVue: function (items) {
      let payload = this.setPlayLoad(items);
      let r = this.springBootVue.getCtrl(payload);
      return r;
    },
    /**
     *
    ...
    setGen: function () {
      ...
      axios
        .post("http://gen.saesolsoft.co.kr/gen/pasgen", {
